...
async def find_flights_on_date(
    ...
    if round_trip and return_date:
        ...
        outbound_data = [FlightData(date=departure_date, from_airport=origin, to_airport=destination, max_stops=max_stops)]
        return_data = [FlightData(date=return_date, from_airport=destination, to_airport=origin, max_stops=max_stops)]
        ...
        _add_flight_metadata(outbound_result.flights, 'outbound', f"{origin}→{destination}")
